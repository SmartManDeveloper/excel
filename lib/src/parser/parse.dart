...
class Parser {
  ...
  void _parseSharedStrings() {
    ...
    if (sharedStrings == null) {
      ...
      if (_excel._xmlFiles.containsKey("xl/_rels/workbook.xml.rels")) {
        ...
        _excel._xmlFiles["xl/_rels/workbook.xml.rels"]?.findAllElements('Relationships').first.children.add(XmlElement(
        ...
        String content = "application/vnd.openxmlformats-officedocument.spreadsheetml.sharedStrings+xml";
        bool contain = true;
    ...
    _excel._sheets.forEach((sheetName, node) {
      ...
      elementNode.findAllElements('mergeCell').forEach((elemen) {
        ...
        if (ref != null && ref.contains(':') && ref.split(':').length == 2) {
          ...
          CellIndex startIndex = CellIndex.indexByString(startCell), endIndex = CellIndex.indexByString(endCell);
  ...
  void _parseStyles(String _stylesTarget) {
    ...
    if (styles == null && _stylesTarget.startsWith("/xl")) {
      styles = _excel._archive.findFile(_stylesTarget.replaceFirst("/xl", "xl"));
    }
    if (styles != null) {
      styles.decompress();
      ...
      document.findAllElements('border').forEach((node) {
        ...
        const List<String> borderElementNamesList = ['left', 'right', 'top', 'bottom', 'diagonal'];
  ...
  void _parseTable(XmlElement node) {
    ...
    if (file == null && target != null && target.startsWith("/xl")) {
      file = _excel._archive.findFile(target!.replaceFirst("/xl", "xl"));
    }
    file!.decompress();
  ...
  void _createSheet(String newSheet) {
    ...
    _excel._xmlFiles['[Content_Types].xml']?.findAllElements('Types').first.children.add(XmlElement(
          ...
          <XmlAttribute>[
            XmlAttribute(
                XmlName('ContentType'), 'application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml'),
